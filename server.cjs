...
app.post("/api/gemini/generate-roadmap", async (req, res) => {
  ...
  const matContext = materials.length > 0 ? `Available Study Materials in Dashboard:
${materials.map((m) => `- ${m.type || "Material"}: ${m.title} (${m.description || ""})`).join("\n")}
              ...
              roadmap: {
                ...
                items: {
                  ...
                  properties: {
                    ...
                    tasks: { type: import_genai.Type.ARRAY, items: { type: import_genai.Type.STRING } }
                  },
            ...
            properties: {
              ...
              questions: {
                ...
                items: {
                  type: import_genai.Type.OBJECT,
                  properties: {
                    text: { type: import_genai.Type.STRING, description: "The question text, expressing a single academic problem clearly." },
                    options: {
                      type: import_genai.Type.ARRAY,
                    ...
                    explanation: { type: import_genai.Type.STRING, description: "Step-by-step mathematical or physical explanation of why this answer is correct." }
                  },
  ...
  if (finalQuestions.length === 0) {
    ...
    for (let i = 0; i < 20; i++) {
      ...
      derivedQuestions.push({
        ...
        correctAnswerIndex: (base.correctAnswerIndex + i) % 4,
        // vary the answers for variety
      ...
      const response = await aiClient.models.generateContent({
        ...
        config: {
          ...
          responseSchema: {
            type: import_genai.Type.OBJECT,
            properties: {
              score: { type: import_genai.Type.INTEGER, description: "Calculated integer score. Must be between 0 and maximum points specified." },
              feedback: { type: import_genai.Type.STRING, description: "A detailed feedback statement containing strengths and 2 precise recommendations for growth." }
            },
